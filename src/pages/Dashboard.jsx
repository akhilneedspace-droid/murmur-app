    ...
    <div className="page" style={{ padding: '0 24px', justifyContent: 'flex-start' }}>
      ...
      {activeSessions.length > 1 && (
        <div style={{ position: 'relative', zIndex: 1, marginBottom: 16, padding: '14px 18px', background: 'rgba(93,202,165,0.08)', border: '1px solid rgba(93,202,165,0.25)', borderRadius: 'var(--radius)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={() => setCurrentListenerSession(activeSessions[0])} style={{ fontSize: 13, color: 'var(--teal)', textDecoration: 'underline', background: 'none', border: 'none', cursor: 'pointer' }}>View chats</button>
        </div>
      )}
  ...
  if (rateLimited) {
    return (
      <div className="page" style={{ padding: '0 28px', justifyContent: 'center', alignItems: 'center', gap: 24, textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 15, color: 'rgba(240,239,232,0.7)', lineHeight: 1.8, maxWidth: 300 }}>You've reached the limit of {DAILY_POST_LIMIT} posts for today. Rest now — we'll be here tomorrow.</p>
  ...
  if (phase === 'acknowledge') {
    return (
      <div className="page" style={{ padding: '0 28px', justifyContent: 'center', alignItems: 'center', gap: 24, textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 13, color: 'rgba(240,239,232,0.45)' }}>Finding a listener for you...</p>
      </div>
    )
  }
...
function ListenerView({ user, myProfile, todayListenerCount, onBack, onComplete }) {
  ...
  async function handleSelectPost(post) {
    if (todayListenerCount >= DAILY_LISTEN_LIMIT) { setShowBurnoutBlock(true); return }
    if (post.is_seed) {
      // Check for existing DB session for this seed post
      ...
      // Create real DB session so it appears in Your Conversations
      const { data: newSession } = await supabase.from('sessions')
        .insert({ post_id: post.id, expresser_id: user.id, listener_id: user.id, status: 'active', is_ai: true })
        .select().single()
      ...
      setActiveSession({ id: newSession?.id ?? `seed-${post.id}`, is_seed: true, post }); setShowEndTip(true); return
    }
    ...
    const { data: existing } = await supabase.from('sessions').select('*').eq('post_id', post.id).eq('listener_id', user.id).maybeSingle()
...
function PastChatsView({ chats, userId, onOpen, onDelete, onBack }) {
  ...
  const myListening = chats.filter(c => c.listener_id === userId && c.expresser_id !== userId)
...
function ChatView({ sessionId: initialSessionId, isExpresser, isSeedSession, isAISession, post, myProfile, currentUserId, preloadedOtherProfile, allListenerSessions, newListenerNotif, onNewListenerDismiss, onSwitchListener, showEndTip, onEndTipDismiss, onBack, onEnd }) {
  ...
  useEffect(() => {
    ...
    if (isSeedSession) {
      // If we have a real DB session ID, load from DB; otherwise use in-memory store
      if (sessionId && !String(sessionId).startsWith('seed-')) {
        async function loadSeedFromDB() {
          ...
          const msgs = data || []
          if (msgs.length === 0) {
            // First open — show the post content as opening message
            const opening = { id: `seed-open-${sessionId}`, sender_id: 'other', content: post?.content ?? '', created_at: new Date().toISOString() }
            setMessages([opening])
          } else {
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `session_id=eq.${sessionId}` },
        (payload) => {
          ...
          if (msg.content?.startsWith('__system__:')) {
            ...
            setMessages(m => m.find(x => x.id === msg.id) ? m : [...m, msg]); return
          }
  ...
  async function send() {
    ...
    if (isAIChat) {
      setAiThinking(true)
      // IMPORTANT: derive role from sender_id, not m.role (which doesn't exist)
      ...
      setMessages(withAI)
      // Persist messages to DB so they survive navigation
      if (sessionId && !String(sessionId).startsWith('seed-')) {
        await supabase.from('messages').insert([
          { session_id: sessionId, sender_id: currentUserId, content },
          { session_id: sessionId, sender_id: 'ai-bot', content: aiText }
        ])
      } else if (isSeedSession) {
        const pid = String(sessionId).replace('seed-', '')
  ...
  if (showRating) return <RatingScreen onSubmit={async (r) => { if (!isAIChat && sessionId) await supabase.from('sessions').update({ rating: r }).eq('id', sessionId); onEnd?.() }} onSkip={() => onEnd?.()} />
  if (ended) {
    return (
      <div className="page" style={{ padding: '0 28px', justifyContent: 'center', alignItems: 'center', gap: 24, textAlign: 'center' }}>
