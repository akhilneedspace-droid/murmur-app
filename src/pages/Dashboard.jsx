...
const SEED_POSTS = [
  { id: '00000000-0000-0000-0000-000000000010', content: "I've been really hard on myself lately. Like nothing I do is ever enough, no matter how hard I try.", emotion_tag: 'overwhelmed', is_anonymous: false, created_at: new Date(Date.now() - 4 * 60000).toISOString(), profiles: { full_name: 'Priya', avatar_url: null }, is_seed: true },
    ...
    { id: '00000000-0000-0000-0000-000000000012', content: "I'lately..", emotion_tag: 'anxiety', is_anonymous: false, created_at: new Date(Date.now() - 4 * 60000).toISOString(), profiles: { full_name: 'Ashley', avatar_url: null }, is_seed: true },
...
function ListenerView({ user, myProfile, todayListenerCount, onBack, onComplete }) {
 ...
 async function handleSelectPost(post) {
  ...
  if (existing) {
    ...
    setActiveSession({ 
      ...existing, 
      expresser_id: existing.expresser_id, // This ensures the ID is preserved
      post: { ...post, id: cleanId }, 
      is_seed: post.is_seed || existing.expresser_id === '00000000-0000-0000-0000-000000000001'
    });
...
async function send() {
  ...
  // 1. Create a local temp message for immediate UI update
  ...
  // Use sessionId (passed from props as activeSession)
  ...
  try {
    ...
    if (!activeSessionId && post) {
      ...
      if (existing) {
        ...
        const { data: newS } = await supabase
          ...
          .select().single();
        if (newS) activeSessionId = newS.id;
      }
      
      // Update the parent state so we don't have to do this check again
    ...
    // Replace temp message with the database one to prevent "doubling" on refresh
    setMessages(m => m.map(msg => msg.id === tempId ? { ...msg, id: inserted.id } : msg));

    // 4. AI TRIGGER LOGIC
    // Determine role: If it's a seed or the post user is the AI, it's an expresser
    const isAiExpresser = isSeedSession || post?.is_seed || post?.user_id === AI_ID;
    const reallyIsAIChat = isAISession || isAiExpresser;

    if (reallyIsAIChat) {
      setAiThinking(true);
      
      // Use the logic we established: AI ID is always 'assistant'
      ...
      const aiRole = isAiExpresser ? 'expresser' : 'listener';
